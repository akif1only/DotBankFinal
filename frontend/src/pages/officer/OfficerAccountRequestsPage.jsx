...
export default function OfficerAccountRequestsPage() {
  const { actor } = useAuth();
  const [requests, setRequests] = useState([]);
  const [loading, setLoading] = useState(true);
  const [acting, setActing] = useState(null);

  const load = () => api.get("/account-requests")
  ...
  const act = async (id, action) => {
    setActing(id + action);
    try {
      await api.post(`/account-requests/${id}/${action}`, { actorName: actor?.id });
      load();
    } finally {
      setActing(null);
    }
      ...
      <div className="mx-auto max-w-4xl px-5 py-8">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}>
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h1 className="font-display text-2xl font-bold text-ink dark:text-inkDark">Account Requests</h1>
          ...
          {loading ? (
                ...
                <div key={i} className="rounded-card bg-white dark:bg-surfaceDark border border-black/5 dark:border-white/8 p-5 animate-pulse">
                  <div className="flex justify-between">
                    <div className="flex gap-8">
                      {[80,100,90].map((w) => <div key={w} className="space-y-2"><div className="h-3 rounded bg-black/5 dark:bg-white/5" style={{ width: w }} /><div className="h-2.5 rounded bg-black/5 dark:bg-white/5" style={{ width: w * 0.7 }} /></div>)}
                    </div>
            ...
            <div className="rounded-card bg-white dark:bg-surfaceDark border border-black/5 dark:border-white/8 shadow-card dark:shadow-cardDark p-16 text-center">
              ...
              <p className="text-sm text-ink-muted dark:text-inkDark-muted">No pending account requests.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {requests.map((r, i) => (
                ...
                >
                  <div className="flex flex-wrap items-center justify-between gap-4">
                    <div className="flex flex-wrap gap-6">
                      <div>
                        <p className="text-xs font-semibold text-ink-muted dark:text-inkDark-muted uppercase tracking-wider">User ID</p>
                        <p className="text-sm font-bold text-ink dark:text-inkDark">{r.user_id}</p>
                      </div>
                      <div>
                        <p className="text-xs font-semibold text-ink-muted dark:text-inkDark-muted uppercase tracking-wider">Account type</p>
                        <p className="text-sm font-medium text-ink dark:text-inkDark">{r.account_type}</p>
                      </div>
                      <div>
                        <p className="text-xs font-semibold text-ink-muted dark:text-inkDark-muted uppercase tracking-wider">Initial deposit</p>
                        <p className="text-sm font-mono font-bold text-ink dark:text-inkDark">${Number(r.initial_deposit).toFixed(2)}</p>
                      </div>
                      <div>
                    ...
                    <div className="flex gap-2">
                      ...
                      >
                        {acting === r.request_id + "approve" ? <span className="h-3.5 w-3.5 rounded-full border-2 border-successDark/30 border-t-successDark animate-spin" /> : <Check size={15} />}
                        Approve
                      </motion.button>
                      <motion.button
                        whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }}
                      ...
                      >
                        {acting === r.request_id + "deny" ? <span className="h-3.5 w-3.5 rounded-full border-2 border-dangerDark/30 border-t-dangerDark animate-spin" /> : <X size={15} />}
                        Reject
                      </motion.button>
                    </div>
                  </div>
